...
- `parse`：提交后台 PDF 入库任务，使用 PDFBox 正文抽取、按页切块、写入 `paper_chunks` 并生成向量索引，同时记录入库节点 span。
- `embedding`：Spring AI embedding 或本地 hashing embedding、pgvector 写入和向量召回。
- `agent`：多 Agent Lite 编排、会话、问答、来源片段和历史记录。
- `agent.evaluation`：Agent 评测集和回归样本管理，支持从 RAG Trace 沉淀可复用质量基线。
...
Agent 评测集是独立于在线问答链路的质量基线层。`evaluation_datasets` 保存单篇或全库范围的回归集元数据，`evaluation_cases` 保存问题、期望答案、期望来源 JSON、标签、难度，以及可选的来源用户、文献、问答记录和 RAG Trace 关联。管理员可以手工创建样本，也可以从高价值 `rag_traces` 一键沉淀样本；从 Trace 创建时，系统会复用 Trace 的问题、范围、用户和文献，并在未覆盖时读取关联 `chat_records.answer` 和 `chat_records.sources_json` 作为期望答案和来源。

当前评测集先提供数据集管理、样本分页浏览、启停和审计记录，为后续批量运行评测、对比模型路由、回归 Prompt 变更和分析工具召回质量做准备。它不影响现有 `AgentPipeline` 的实时响应路径，因此不会增加普通用户问答延迟；后续可在此基础上新增离线 Evaluation Runner，把同一批样本跑过不同模型目标或 Pipeline 配置，并把准确性、引用覆盖和质量评审结果回写为报告。

## Runtime Observability

RAG Trace 采用“概览快照 + 分页检索 + 单条详情”的轻量可观测设计。`AgentPipeline` 和各检索后处理器在运行时把节点 span、检索通道、后处理器、查询改写、术语扩展、工具执行、意图引导、长期摘要、来源上下文预算、答案质量和耗时写入 `rag_traces`；限流拒绝也会落入失败 Trace，便于把 429 与具体用户、会话范围和问题关联起来。`AdminService` 统一解析 JSONB 字段，既向 `/api/admin/overview` 提供最近 7 天运营趋势、最近 Trace、最近解析任务、聊天限流状态和活跃流式任务，也向 `/api/admin/rag-traces` 提供按状态、范围、会话 ID 和关键词过滤的分页查询，通过 `/api/admin/parse-jobs` 提供按状态和关键词过滤的解析任务分页查询，并通过 `/api/admin/agent-pipeline/nodes` 把 `AgentPipeline` 中的节点定义、`agent_pipeline_node_settings` 启停状态与历史 `node_spans_json` 聚合成节点目录和运行画像，通过 `/api/admin/ingestion-pipeline/nodes` 把 `IngestionPipelineCatalog` 中的入库节点定义与历史 `parse_jobs.node_spans_json` 聚合成入库节点目录和运行画像，通过 `/api/admin/retrieval-channels` 把 `RetrievalChannel` 注册表与历史 `retrieval_channels_json` 聚合成检索通道目录和召回画像，通过 `/api/admin/retrieval-processors` 把 `RetrievalPostProcessor` 注册表与历史 `retrieval_processors_json` 聚合成后处理器目录和输入输出画像，通过 `/api/admin/agent-tools` 把 `AgentToolRegistry` 中的工具定义、`agent_tool_settings` 启停与最小调用角色、历史 `tool_executions_json` 聚合成工具目录和调用画像，通过 `/api/admin/agent-tool-executions` 把历史 `tool_executions_json` 展开成可按工具、状态和关键词过滤的调用审计台账，通过 `/api/admin/audit-logs` 把后台配置变更、权限调整、模型熔断复位、流式任务取消和知识片段治理动作展开成可追责的管理员操作审计台账，通过 `/api/admin/chunks` 从 `paper_chunks` 暴露可分页检索的知识片段治理视图。管理台的运营趋势面板会按天展示新增用户、文献、问答量、成功/失败 Trace、解析任务和平均耗时；活跃流式任务面板可以观察当前仍在排队或运行的 SSE 请求、所属用户、问题、单篇/全库范围和会话编号，并允许管理员强制停止异常占用；Trace Explorer 可以展开单条链路，查看检索式、改写、工具、引导、记忆、上下文预算、质量评估、通道、后处理器和节点耗时；解析任务 Explorer 可以追查历史 PDF 入库任务、失败原因和每个入库节点耗时；Agent Pipeline 节点目录用于观察问答链路顺序、节点职责、启停状态、成功/失败/跳过次数、平均耗时和最近运行时间，入库 Pipeline 节点目录用于观察 PDF 解析链路的节点职责、长期失败风险和耗时热点，检索通道与后处理器目录用于观察向量/关键词召回、通道融合、规则精排、模型重排、多样性重排和结果截断的候选规模、输入输出规模、成功/失败次数和最近运行时间，Agent 工具目录用于观察工具触发规则、启停状态、最小调用角色、成功/失败次数、平均耗时和最近调用时间，意图路由维护用于把特定路由绑定到已注册工具，Agent 工具调用审计用于追查单次工具调用的触发问题、Trace 编号、调用用户、结果摘要、失败原因和耗时，管理员操作审计用于追查某次后台治理动作由谁执行、影响哪个资源和写入了哪些非敏感详情，知识片段治理用于检查入库后的正文预览、页码、chunk 序号、向量化状态和启用状态；被禁用的 Agent 增强节点保留在目录中但后续链路记录为 `SKIPPED`，被禁用或当前用户角色不足的工具保留在目录中但不参与后续问答链路，即使某条意图路由绑定了该工具也不会绕过治理，被禁用的片段保留在后台治理视图中但 `KeywordRetrievalChannel` 与 `PaperChunkVectorRepository` 都会排除它，便于排查召回不足、问题过泛、生成过慢、质量风险、工具异常、入库异常、限流拒绝和模型路由异常。

## Ingestion Observability

PDF 入库采用轻量后台队列：`POST /api/papers/{id}/parse` 只负责校验归属、创建 `QUEUED` 解析任务、把文献置为 `PARSING` 并在事务提交后投递到 `parseTaskExecutor`；后台任务再进入 `RUNNING`，依次执行 `prepare -> fetch-pdf -> parse-pdf -> persist-chunks -> index-embeddings -> finalize`。每个节点记录名称、状态、耗时和错误摘要，并保存到 `parse_jobs.node_spans_json`；成功后任务为 `SUCCESS`、文献为 `INDEXED`，失败后任务为 `FAILED`、文献为 `FAILED`。同一用户同一文献已有 `QUEUED` 或 `RUNNING` 任务时，重复解析请求会直接返回已有任务状态，避免重复入库；取消解析会拒绝正在排队或运行的任务。管理员后台的解析任务 Explorer 会显示这些节点标签，并支持按状态、文件名、文献标题、用户名、节点内容或错误信息检索，用于定位是排队、文件读取、PDF 文本抽取、切块写库还是 embedding 索引慢或失败。
