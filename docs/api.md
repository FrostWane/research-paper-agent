...
`AnswerAgent` 会优先使用启用的默认模板；如果没有默认模板，则使用排序最靠前的启用模板；如果数据库无可用模板，则使用内置兜底模板。支持占位符：`{{scope}}`、`{{paper_metadata}}`、`{{answer_strategy}}`、`{{answer_contract}}`、`{{conversation_history}}`、`{{tool_context}}`、`{{question}}`、`{{sources}}`。其中 `{{tool_context}}` 来自 `ToolExecutionNode` 的成功工具结果，可用于回答文献库统计、解析状态和系统运营类问题。
