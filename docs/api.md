...
`provider` 支持 `ENV` 和 `OPENAI_COMPATIBLE`。`ENV` 表示使用当前环境变量和 Spring AI 配置；`OPENAI_COMPATIBLE` 会按 `baseUrl + /v1/chat/completions` 调用兼容接口。`taskType` 支持 `GENERAL`、`ANSWER_GENERATION` 和 `QUERY_REWRITE`，用于让回答生成、查询改写等节点选择不同模型；路由时会先按任务类型尝试启用目标，再尝试 `GENERAL` 通用目标。模型路由会按启用目标的 `priority` 升序逐个尝试，失败会记录到 `model_invocations` 并继续尝试下一个目标，全部失败后才走 fallback。更新时 `apiKey` 留空表示保留原密钥。
...
`candidateLimit` 控制每个检索通道的候选召回上限，`resultLimit` 控制最终返回来源数，`sourceExcerptChars` 控制来源卡片摘录长度，`vectorWeight` 和 `keywordWeight` 控制通道融合权重，`memoryHistoryTurns` 和 `memoryMaxChars` 控制同范围历史问答注入 Prompt 的轮数和字符上限，`queryRewriteEnabled` 和 `queryRewriteMaxSubQuestions` 控制查询改写与子问题拆分。
